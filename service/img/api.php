...
function login($username, $pass) {
	$result = query("SELECT userID, username,fullName,gender,bio,year,college FROM login WHERE username='%s' AND userpass='%s' ", $username, $pass);
	if (count($result['result'])>0) {
		//authorized
		$_SESSION['userID'] = $result['result'][0]['userID'];
...
function showInfo($username){
	$result = query("SELECT , username,email FROM login WHERE username='%s' ", $username;
	if ($result['error']) {
...
function AddTradeToList($username,$have,$exchange,$emailPrivacy,$note){



	$result = query("INSERT INTO trade_List(username,have,exchange,emailPrivacy,note) VALUES('$username','$have','$exchange','$emailPrivacy','$note')");
		if ($res['error']) {
	//echo "error when find username";
			print json_encode($result);
		}else{	
